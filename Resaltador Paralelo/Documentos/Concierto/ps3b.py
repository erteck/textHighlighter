...
class Patient(object):
    ...
    def update(self):
        ...
        integer)
        """
        new = self.viruses.copy()
        for i in range(0,len(self.viruses)):
             if self.viruses[i].doesClear():
                 new.remove(self.viruses[i])
        self.viruses = new
        
        self.popDensity = self.getTotalPop()/self.maxPop
        
        add_childslist = self.viruses.copy()
        for i in range(0,len(self.viruses)):
            try:
                son = self.viruses[i].reproduce(self.popDensity)
                add_childslist.append(son)
            except NoChildException:
                continue
        self.viruses = add_childslist
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for trial in range(0,numTrials):
        ...
        for timestep in range(0,300):
            values[timestep] += patient.update()
    for i in range(0,len(values)):
        values[i] = values[i]/numTrials
    
    pylab.plot(values, label = "SimpleVirus")
...
#simulationWithoutDrug(13, 1000, 0.1, 0.01, 100)
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        for key in activeDrugs:
            if self.resistances[key] == False:
                raise NoChildException()

        repr_prob =  self.maxBirthProb * (1 - popDensity)
        if random.random() <= repr_prob:
            oneminusmutprob = 1 - self.mutProb
            resistances = {}
            for key in list(self.resistances.keys()):
                ...
                elif self.resistances[key] == False:
                    if random.random() <= self.mutProb:
                        resistances[key] = True
                    else:
                        resistances[key] = False
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        for i in range(0,len(self.viruses)):
            try:
                son = self.viruses[i].reproduce(self.popDensity,self.prescription)
                add_childslist.append(son)
            except NoChildException:
                continue
        self.viruses = add_childslist
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for trial in range(0,numTrials):
        ...
        for timestep in range(0,150):
            values[timestep] += patient.update()
            resist_values[timestep] += patient.getResistPop(['guttagonol'])
    ...
    for i in range(0,len(values)):
        values[i] = values[i]/numTrials
        resist_values[i] = resist_values[i]/numTrials
    
    pylab.plot(values, label = "Total Population")
    pylab.plot(resist_values, label = "Non Resistant Population")
