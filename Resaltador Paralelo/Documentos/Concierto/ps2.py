...
class RectangularRoom(object):
    ...
    def cleanTileAtPosition(self, pos):
        ...
        pos: a Position
        """
        if (math.floor(pos.getX()),math.floor(pos.getY())) not in self.clean_tiles:
            self.clean_tiles.append((math.floor(pos.getX()),math.floor(pos.getY())))
...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        while self.room.isPositionInRoom(new) != True:
            new_direction = random.randrange(0,359)
            self.setRobotDirection(new_direction)
            new = self.getRobotPosition().getNewPosition(new_direction,self.speed)
        self.setRobotPosition(new)
        self.room.cleanTileAtPosition(self.getRobotPosition())
        
        
#room = RectangularRoom(8,8)
#f = StandardRobot(room,1)        
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for trial in range(0,num_trials):
        ...
        meantimesteps.append(time_steps)
        anim.done()
    
    return sum(meantimesteps)/len(meantimesteps)
